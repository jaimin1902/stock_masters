  ...
  static async findAll(filters = {}) {
    let query = `
      SELECT 
        ...
        -- Reference numbers and details from related tables
        CASE 
        ...
        CASE 
          ...
          WHEN mh.transaction_type IN ('transfer_out', 'transfer_in') THEN NULL
          WHEN mh.transaction_type = 'adjustment' THEN NULL
          ELSE NULL
        END as contact_name,
        -- From/To information
        CASE 
          WHEN mh.transaction_type = 'receipt' THEN s.name
          WHEN mh.transaction_type = 'delivery' THEN w.name
          WHEN mh.transaction_type = 'transfer_out' THEN w.name
          WHEN mh.transaction_type = 'transfer_in' THEN wf.name
          WHEN mh.transaction_type = 'adjustment' THEN w.name
          ELSE NULL
        END as from_location,
        CASE 
          WHEN mh.transaction_type = 'receipt' THEN w.name
          WHEN mh.transaction_type = 'delivery' THEN d.customer_address
          WHEN mh.transaction_type = 'transfer_out' THEN wt.name
          WHEN mh.transaction_type = 'transfer_in' THEN w.name
          WHEN mh.transaction_type = 'adjustment' THEN w.name
          ELSE NULL
        END as to_location,
        -- Schedule dates
        CASE 
          WHEN mh.transaction_type = 'receipt' THEN r.schedule_date
          WHEN mh.transaction_type = 'delivery' THEN d.schedule_date
          WHEN mh.transaction_type IN ('transfer_out', 'transfer_in') THEN NULL
          WHEN mh.transaction_type = 'adjustment' THEN NULL
          ELSE NULL
        ...
        -- Status
        CASE 
          WHEN mh.transaction_type = 'receipt' THEN r.status
          WHEN mh.transaction_type = 'delivery' THEN d.status
          WHEN mh.transaction_type IN ('transfer_out', 'transfer_in') THEN t.status
          WHEN mh.transaction_type = 'adjustment' THEN 'completed'
          ELSE NULL
        END as status
